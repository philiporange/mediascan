...
class TestInterpreter(unittest.TestCase):
    ...
    def test_find_year(self):
        ...
        self.assertEqual(
            self.interpreter.find_year("Movie Title")["value"], None
        )

    def test_find_episode(self):
        self.assertEqual(
            self.interpreter.find_episode("S01E05")["value"], (1, 5)
        )
        self.assertEqual(
            self.interpreter.find_episode("1x05")["value"], (1, 5)
        )
        self.assertEqual(
    ...
    def test_find_season(self):
        ...
        self.assertEqual(
            self.interpreter.find_season("No season")["value"], None
        )
    ...
    def test_find_resolution(self):
        self.assertEqual(
            self.interpreter.find_resolution("1080p")["value"], "1080p"
        )
        self.assertEqual(
            self.interpreter.find_resolution("720p")["value"], "720p"
        )
        self.assertEqual(
    ...
    def test_find_source(self):
        self.assertEqual(
            self.interpreter.find_source("BluRay")["value"], "bluray"
        )
        self.assertEqual(
            self.interpreter.find_source("WEB-DL")["value"], "web"
        )
        self.assertEqual(
            self.interpreter.find_source("No source")["value"], None
        )

    def test_is_proper_or_repack(self):
        self.assertTrue(
            self.interpreter.is_proper_or_repack("PROPER")["value"]
        )
        self.assertTrue(
            self.interpreter.is_proper_or_repack("REPACK")["value"]
        )
        self.assertFalse(
            self.interpreter.is_proper_or_repack("Not proper")["value"]
        )
